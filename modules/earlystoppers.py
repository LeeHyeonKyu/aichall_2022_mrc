...
class EarlyStopper:
    def __init__(
        self, patience: int, mode: str, logger: logging.RootLogger = None
    ) -> None:
    ...
    def check_early_stopping(self, loss: float) -> None:
        ...
        if loss > self.best_loss:
            ...
            if self.logger is not None:
                self.logger.info(
                    f"Early stopper, counter {self.patience_counter}/{self.patience}, best:{abs(self.best_loss)} -> now:{abs(loss)}"
                )
